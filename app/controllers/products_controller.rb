...
class ProductsController < ApplicationController
  ...
  def index
    @products = Product.includes(:category).published.accessible_by(current_user).ordered
  ...
  def show
    ...
    unless @product.accessible_by?(current_user)
      redirect_to products_path, alert: 'У вас нет доступа к этому продукту'
      return
    end
